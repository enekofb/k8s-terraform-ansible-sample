...
data "template_file" "ssh_cfg" {
    ...
    depends_on = ["aws_instance.etcd", "aws_instance.master", "aws_instance.worker"]
    vars {
      ...
      controller0_ip = "${aws_instance.master.0.public_ip}"
      controller1_ip = "${aws_instance.master.1.public_ip}"
      controller2_ip = "${aws_instance.master.2.public_ip}"
